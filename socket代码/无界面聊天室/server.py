...
def newConnection(ss):
    ...
    try:
        # 向client端发送欢迎信息
        client_conn.send(str.encode("welcome to chatroom,pls set up your nick name!"))
        client_name = client_conn.recv(1024).decode() #接收client发来的昵称,最大接收字符为1024
        inputs.append(client_conn)
        fd_name[client_conn] = client_name  # 将连接/连接名 加入键值对
        client_conn.send(str.encode("current members in chatroom are: %s" % fd_name.values()))
        # 向所有连接发送新成员加入信息
        for other in fd_name.keys():
            if other != client_conn and other != ss:
                other.send(str.encode(fd_name[client_conn]+" joined the chatroom!"))
    ...
    while True:
        ...
        for r in rlist:
            if r is ss:  # server socket, 表示有新的client连接请求
                ...
                disconnect = False
                try:
                    data = r.recv(1024).decode()  #接收data
